...
from planning.rdf_2d import RDF_2D_Planner
from planning.armtd_2d import ARMTD_2D_planner
from environments.wrappers.monitoring.video_recorder import VideoRecorder
import os


def evaluate_planner(planner, planner_name='rdf', device='cpu', n_envs=1000, n_steps=150, n_links=2, n_obs=3, record_episodes=False, video=False, buffer_size=0.0, time_limit=0.5):
    t_armtd = 0.0
    num_success = 0
    num_collision = 0
    num_no_solution = 0
    ...
    video_folder = f'planning_videos/{planner_name}/2d{n_links}links'
    if record_episodes:
        success_episodes = set()
    assert 'rdf' in planner_name or 'armtd' in planner_name, f"Only support rdf or armtd, while received {planner_name}"
        
    for i_env in tqdm(range(n_envs)):
        set_random_seed(i_env)
        env = Arm_2D(n_obs=n_obs, T_len=24, max_episode_steps=n_steps, n_links=n_links, scale_down=n_links*1.2)
        t_curr_trial = []

        if 'armtd' in planner_name:
            planner = ARMTD_2D_planner(env, device=device)
        if video:
            video_path = os.path.join(video_folder, f'video{i_env}')
            video_recorder = VideoRecorder(env, video_path, frame_rate=3, format='gif')
        was_stuck = False
        for i_step in range(n_steps):
            ts = time.time()
            k0 = torch.zeros(n_links)
            if 'rdf' in planner_name:
    ...
    # with open(f"planning_results/2d{n_links}links{n_obs}obs/{planner_name}_time_2d{n_links}links{n_envs}trials{n_obs}obs{n_steps}steps.npy", 'wb') as f:
    #     np.save(f, np.array(t_armtd_list))
    with open(f"planning_results/2d{n_links}links{n_obs}obs/{planner_name}_stats_2d{n_links}links{n_envs}trials{n_obs}obs{n_steps}steps.json", 'w') as f:
        if record_episodes:
...
if __name__ == '__main__':
    ...
    from environments.arm_2d import Arm_2D
    
    params = read_params()
    device = torch.device(params.device)

    print(f"Running {params.n_envs}trials of 2D{params.n_links}Links{params.n_obs}obs with {params.n_steps} step limit and {params.time_limit}s time limit each step")
    print(f"Using device {device}")
    planner_name = params.planner
    model_path = params.model
            
    planning_result_dir = f'planning_results/2d{params.n_links}links{params.n_obs}obs'
    ...
    stats = {}
    if planner_name == 'rdf' or planner_name == 'both':
        if model_path == '':
            model_path = f'trained_models/RDF2D/{params.n_links}links/2d-signed-convexhull.pth' 
        rdf_planner = RDF_2D_Planner(model_path=model_path, device=device, n_links=params.n_links, n_dims=params.n_dims)
    ...
    if planner_name == 'armtd' or planner_name == 'both':
